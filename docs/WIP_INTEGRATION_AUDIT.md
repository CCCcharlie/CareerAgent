...
本文件可以归档，不需要长期维护——它记录的是"整合前的事实快照"。
...
| 功能 | 涉及文件 | 状态 |
|---|---|---|
| 多简历加载 + 五维评分 | `requirements.txt`（新增 python-docx）、`main.py`（`_load_multiple_resumes`/`_read_docx_async`）、`agent/matcher.py`（`resume_texts` 参数） | 已核实，签名和逻辑与 README.md/OPTIMIZATION.md 描述一致 |
| 单次推理优化 | `agent/matcher.py`（`score_job`/`_parse_json`） | 已核实，`weighted_score` 真实存在并优先于 `score` |
| 交互式多次搜索 | `main.py`（`run()` 循环、`_show_help`） | 已核实实现，但**无跨轮次去重逻辑**，见下方"已知遗留问题" |
| extractor 集成 | 新增 `agent/extractor.py`，`agent/browser.py`（`get_job_detail_text`/`get_job_url`），`main.py`（`_crawl_and_score`） | 已核实 |
| URL 校验修复 | `FIX_URL_VALIDATION.md` | **文档描述（黑名单+四级降级）与实际代码（正向匹配）不符**，需在整合时改写文档而非照抄实现 |
| 视觉模型超时 | `agent/vision.py` | 120s → 300s，改动极小 |
| 浏览器 profile 可配置 | `agent/browser.py` | 新增 `CAREER_AGENT_PROFILE_DIR` 环境变量覆盖 |

## 已确认的 bug（不是可选优化，是需要修复的真实问题）

**main.py 丢弃 matcher 计算出的三个字段**（第277-278行）：
...
五维评分这个功能"算了但没存"，等于白算。整合时必须在 Phase 6a
一并修复，并配一个专门验证字段没被丢弃的回归测试。
