...
`docs/` 下的独立文档（比如 `INTEGRATION_PLAN.md`、
`DOM_EXTRACTION_RUNSHEET.md`），完成任务后可以归档，不要把一次性
施工步骤永久写进本文件。

当前活跃任务可能不止一份施工文档，且会随时间变化——开始任务前先
看 `docs/` 目录下有哪些文档、各自顶部的进度行，判断哪一份是当前
真正在进行的工作，不要假设只有一份或者固定是某个文件名，本文件
不维护这个信息（会过期）。
...
**分阶段任务**：如果当前任务有 Runsheet、Integration Plan 或其他
Step / Phase 施工文档，开始前必须读取当前 Step / Phase，并将其视为
本轮工作的执行边界；范围外问题只记录，不顺手处理。

**AGENTS.md 更新边界**：本文件只记录长期有效的工程规则、架构边界和
已经确认的长期决策。只有当前任务产生了新的长期性架构判断，或真实
证据证明本文件已有规则已经不准确时，才修改 `AGENTS.md`。普通功能
实现、临时诊断、当前进度、一次性施工细节和未验证假设不要写入本
文件——但"某个旧假设已被真实证据证伪"这件事本身属于确认过的事实，
应该记录，即使替代方案还在验证中（措辞上要区分"旧方案已证伪"和
"新方案已确立"，后者要等真正验证通过才能写）。
...
- **候选 URL 的优先级和兜底行为已经有测试确认**：候选顺序是
  ...
  场景证明需要更严格的关联校验，否则保留现有顺序和行为。（注意：
  这条是`extract_url()`提取单个岗位详情URL的机制，和下面这条"岗位
  列表卡片选择器"是完全不同的两套东西，不要混淆。）
...
**诊断/只读排查类任务**：如果一个任务只是加临时日志、核实假设、
读代码定位根因，没有产生正式代码改动，不需要为了"有产出"而制造
空 commit——诊断结论汇报清楚即可，等真正要修复时再进入正式的
开发+提交流程。

**长内容一律写文件，不要打印到终端**：诊断/调试过程中如果需要
输出较长内容（完整HTML片段、完整JSON数组、大段diff），写入文件
（比如 `docs/debug/` 下）让用户自己打开看，不要直接打印到终端——
终端输出在长内容下容易被截断，这个项目已经因此重复浪费过排查
轮次。
